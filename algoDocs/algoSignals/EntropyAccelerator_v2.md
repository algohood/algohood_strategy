...
- 特征计算：
  ...
  * 状态变量更新：
    ...
    初始化：
    - 在首个5个处理周期中存储原始ΔH值（不足5个周期时延迟信号生成）
    - 初始μ = 前5周期ΔH的算数平均
    - 初始σ = sqrt(Σ(ΔH_i - μ)^2/(5-1))
  * 补充特征：
    波动率σ_v = 最近K=50周期收益率的滚动标准差 
- 复杂度分析：
...
- 阈值参数：
  ...
  3. η=0.15×σ_v/Kσ_r（动态速率阈值），σ_r=0.05为风险系数
- 模式参数：
  1. N=5（经验最优滚动窗口）
  2. q=1.7（最优非广延性参数）
  3. K=50（波动率观测窗口）
- 调优建议：
  q值在[1.5,2.0]区间调整，增加可提高黑天鹅捕捉灵敏度
  N在[3,7]区间选择，交易所延迟高时建议取较小值
  K根据标的波动特性选择，高波动品种建议K=30
